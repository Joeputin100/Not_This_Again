...
- [ ] **Step 1:** Add the line banks to `en.json` (verbatim text from spec §6) under a `granny` object: `granny_dialog_intro` (3), `granny_dialog_chatter` (5), `granny_dialog_chase` (2), `granny_dialog_win_full` (1), `granny_dialog_win_partial` (1), `granny_dialog_win_zero` (1), `granny_dialog_cooldown` (1), `granny_dialog_cackle` (4).
- [ ] **Step 2:** Create `tools/gen_granny_vo.py` modeled on `tools/gen_rustler_vo.py`: voice `vFLqXa8bgbofGarf6fZh`, model `eleven_v3`, `apply_text_normalization="on"`, `SLOTS` mapping the en.json keys → slugs (`intro/chatter/chase/win_full/win_partial/win_zero/cooldown/cackle`), a `PERFORMANCE` dict tagging each line for a chatty sweet-but-sly cackle (`[cackles]`, `[laughs]`, `[whispers]`, `[mischievously]`; the `cackle` bank leans hard on `[cackles]`/`[laughs]`), output `godot/assets/audio/characters/granny_<slot>_<n>.mp3`.

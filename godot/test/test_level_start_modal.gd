...
func test_show_level_sets_title_and_goal():
	modal.show_level("MINE SHAFT MAYHEM", "Clear 60 outlaws, then defeat The Candy Rustler!")
	assert_true(modal.visible, "modal should be visible after show_level")
	assert_eq(modal._header_label.text, "MINE SHAFT MAYHEM")
...
func test_nodes_resolve():
	modal.show_level("X", "Y")
	assert_not_null(modal.get_node_or_null("Panel/HeaderPill/HeaderLabel"))
	assert_not_null(modal.get_node_or_null("Panel/GoalLabel"))
